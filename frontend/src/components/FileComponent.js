...
const File = () => {
    ...
    useEffect(() => {
        fetchFiledata();
    }, [page, selectedDate, searchQuery, filter]);
    ...
    const fetchSubCategorydata = async (categoryId) => {
        try {
            ...
            if (response) {
                ...
                setSubCategorydata(response.data.getsubcategoriesbystatus);
    ...
    const fetchCategorydata = async () => {
        try {
            ...
            if (response) {
                ...
                setCategorydata(response.data.categoriesbystatus);
                setTotalPages(response.data.totalPages);
                console.log('SubCategory retrieved successfully:', response.data);
            } else {
                console.error('Failed to retrieve SubCategorys. Status code:', response.status);
    ...
    const handleChange = (event) => {
        ...
        if (name === 'category') {
            ...
            setFormData(prevFormData => ({ ...prevFormData, category: selectedCategory ? selectedCategory.category : '' }));
        }
    ...
    const handleFavClick = async (fileId) => {
        try {
            ...
            const response = await axiosInstance.post('/file/savefav', null, {
                params: {
                        ...
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                            {isAdmin ?
                                <Button onClick={handleOpen}>
                                    <CreateBtn text={"Create New Files"} />
                                </Button> : null}
                            <div>
                                    ...
                                    <Box sx={{
                                        ...
                                        <form onSubmit={isedit ? handleUpdateSubmit : handleSubmit}>
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item xs={12} sm={6}>
                                                    <FormControl fullWidth>
                                                        <InputLabel>Notification Medium</InputLabel>
                                                        <Select
                                    ...
                                    {Filedata && Filedata.length > 0 ? (
                                        Filedata.map((File, index) => (
                                            <TableRow key={File.id}>
                                                ...
                                                <TableCell>{File.title}</TableCell> {/* Use title instead of name */}
                                                ...
                                                {/* <TableCell>{File.Account.name}</TableCell>
                                                ...
                                                <TableCell onClick={() => handleFavClick(File.id)}>
                                                    {favfileid && favfileid.includes(File.id) ? (
