...
const FileUpload = () => {
  ...
  const fetchFileUploads = async () => {
    try {
      const response = await axiosInstance.get("/fileupload/getfileupload");
      setFileUploads(response.data.fileuploaddata);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleViewFileUpload = (FileUpload) => {
    ...
    <div>
            ...
            {FileUploads && FileUploads.length > 0 ? (
              FileUploads.map((item) => (
                <Tr key={item.id}>
                  ...
                  <Td>
                    <Btn style={{ color: "Blue" }} onClick={() => handleViewFileUpload(item)}>View</Btn>
              ...
              <Tr>
                <Td colSpan="10">No fileupload Available</Td>
              </Tr>
            )}
          </Tbody>
        </Table>
      ...
      {showModal && (
        <Modal>
          <ModalContent>
            <Top>
              <h3>{selectedFileUpload.title}</h3>
              <Close onClick={handleCloseModal}><i class='bx bx-x' style={{ fontSize: "30px" }}></i></Close>
            </Top>

            <p>{selectedFileUpload.description}</p>
            <img className="" src={selectedFileUpload.imageUrl}></img>
...
const AddFileUploadContainer = styled.div`
  padding: 20px;
  display: flex;
  align-items: center;
  margin-bottom: 10px;
...
const AddFileUploadButton = styled.form`
  margin: auto;
  display:flex;
  gap:10px;
  width:100%;
